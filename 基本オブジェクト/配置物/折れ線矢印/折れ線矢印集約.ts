...
export class 折れ線矢印集約<座標点T extends 配置物座標点> implements I折れ線矢印集約<座標点T>, I折れ線矢印シリアライズ可能 {
    ...
    public 始点と終点の付箋の接続点を最短のものに切り替える(){
        ...
        if (中点が存在する) {
            ...
            // 接続点からではなく、付箋の重心（中央pos）から方向を計算することで、接続先の変更によるループ（チャタリング）を防止する
            const 始点から次の点への方向ベクトル = 始点の次の点.描画座標点.minus(始点の付箋.矢印接続可能なもの.中央pos.to描画座標点());
            const 終点から前の点への方向ベクトル = 終点の前の点.描画座標点.minus(終点の付箋.矢印接続可能なもの.中央pos.to描画座標点());
            const 始点の最適な接続点 = 始点の付箋.矢印接続可能なもの.対象方向へもっとも成す角が小さい接続点を取得する(始点から次の点への方向ベクトル, this.始点ハンドル.接続点 ?? undefined);
            const 終点の最適な接続点 = 終点の付箋.矢印接続可能なもの.対象方向へもっとも成す角が小さい接続点を取得する(終点から前の点への方向ベクトル, this.終点ハンドル.接続点 ?? undefined);
