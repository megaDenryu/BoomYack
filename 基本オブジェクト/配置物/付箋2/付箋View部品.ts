import { Canvas座標Base, div, DivC, Drag中値, Px長さ, 配置物座標点 } from "SengenUI/index";
...
import type { 自動リサイズ付箋Viewオプション } from "./自動リサイズ付箋Viewオプション";
...
export interface 付箋View部品配線 {
    on選択(e: PointerEvent): void;
    onContextMenu(e: MouseEvent): void;
    onDragStart(): void;
    onDrag(e: Drag中値): void;
    onDragEnd(): void;
    onTextChange(text: string): void;
    onTextCommit(oldText: string, newText: string): void;
    onHeightChange(height: number): void;
    onFocus(): void;
    onLeftResize(e: Drag中値): void;
    onRightResize(e: Drag中値): void;
}
...
export function 付箋View部品を構築する<T extends Canvas座標Base<T> & 配置物座標点>(
    ...
    const コンテンツ = 付箋コンテンツViewを生成する(option.初期コンテンツ, {
        最小高さ: option.minHeight,
        onTextChange: 配線.onTextChange,
        onBlurTextCommit: 配線.onTextCommit,
        onHeightChange: 配線.onHeightChange,
        onFocus: 配線.onFocus,
    ...
    return {
        ...
        コンテンツ,
        ドラッグ操作領域: new 付箋ドラッグ操作領域("付箋をドラッグ").配線する({
            onPointerDown: 配線.on選択, onContextMenu: 配線.onContextMenu,
            onドラッグ開始: 配線.onDragStart, onドラッグ中: 配線.onDrag, onドラッグ終了: 配線.onDragEnd,
        }),
        左ハンドル: ハンドル("左", 配線.onLeftResize),
        右ハンドル: ハンドル("右", 配線.onRightResize),
        接続点: new 矢印接続可能なもの(geometry.接続点(接続点余白), arrowDep, parent),
    };
}

function ハンドル(方向: "左" | "右", onDrag: (e: Drag中値) => void): リサイズハンドル {
    return new リサイズハンドル(方向 === "左" ? "left" : "right")
        .tap(x => x.mouseWife.ドラッグ連動登録({ onドラッグ開始: () => {}, onドラッグ中: onDrag, onドラッグ終了: () => {} }))
