...
export class 付箋設定パネル extends LV2HtmlComponentBase {
    ...
    private create背景色設定(): DivC {
        return (
            div({ class: 設定項目 }).childs([
                label({ class: 設定項目ラベル, text: "背景色" }),
                input({ class: カラー入力, type: "color", value: this._現在の設定.背景色 })
    ...
    private create文字サイズ設定(): DivC {
        return (
            div({ class: 設定項目 }).childs([
                label({ class: 設定項目ラベル, text: "文字サイズ (px)" }),
    ...
    private create文字色設定(): DivC {
        return (
            div({ class: 設定項目 }).childs([
                label({ class: 設定項目ラベル, text: "文字色" }),
                input({ class: カラー入力, type: "color", value: this._現在の設定.文字色 })
                    .addTypedEventListener("input", (e: Event) => {
                        const target = e.target as HTMLInputElement;
