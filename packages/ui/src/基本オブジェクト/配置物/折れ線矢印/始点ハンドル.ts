...
import { I接続点, I接続点と接続可能, I接触点を教えてくれる人, I折れ線矢印集約, I点ハンドル, I線分ハンドル, 接触判定可能な点 } from "../../I配置物";
import { I配置物選択機能集約 } from "../../キャンバス操作/配置物選択管理";
import { 接続参照データ } from "../../描画キャンバス/データクラス";
import { 始点State } from "./折れ線矢印state";
import { 始点ハンドルView } from "./始点ハンドルView";

export class 始点ハンドル<T extends Canvas座標Base<T> & 配置物座標点>
implements I点ハンドル<T>, 接触判定可能な点, I接続点と接続可能 {
    public readonly index = 0;
    public readonly view: 始点ハンドルView;
    private connection: I接続点<T> | null = null;
    public constructor(
        public readonly state: 始点State<T>,
    ...
    public get nextハンドル(): I点ハンドル<T> {
        const handle = this.親の折れ線矢印集約.get点ハンドルByIndex(1);
        if (!handle) throw new Error("存在しない点ハンドルにアクセスしようとしました");
        return handle;
    }
    public get next線分ハンドル(): I線分ハンドル<T> {
        const handle = this.親の折れ線矢印集約.get線分ハンドルByIndex(0);
        if (!handle) throw new Error("存在しない線分ハンドルにアクセスしようとしました");
        return handle;
    }
    ...
    public setPosition(pos: T): this { this.state.setPosition(pos); this.render(); this.next線分ハンドル.render(); return this; }
    ...
    public 接続(point: I接続点<T>): this { this.接続解除(); this.view.見た目を接続状態にする(); this.connection = point; point.接続(this); return this; }
    private 接続解除(): this { this.connection?.接続解除(this); this.connection = null; return this; }
